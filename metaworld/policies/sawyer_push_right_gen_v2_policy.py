...
class SawyerPushRightGenV2Policy():
    def __init__(self, skill='Push right', target_object='milk'):
        self.stage = 0
        ...
        self.action_list.append(MoveTo(self.target_func_move_left, 'gripper_close', error_limit=0.01))
        self.action_list.append(MoveTo(self.target_func_move_left_down, 'gripper_close', error_limit=0.01))
    ...
    def target_func_move_left(self, state):
    ...
    def target_func_move_left_down(self, state):
        target_xyz = state['target_object_xyz'].copy()
        target_xyz[0] -= 0.06
        target_xyz[-1] += 0.06
        target = np.hstack(
