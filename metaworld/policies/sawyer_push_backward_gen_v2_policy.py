...
class SawyerPushBackwardGenV2Policy():
    def __init__(self, skill='Push backward', target_object='milk'):
        self.stage = 0
        ...
        self.action_list.append(MoveTo(self.target_func_move_front, 'gripper_close', error_limit=0.01))
        self.action_list.append(MoveTo(self.target_func_move_front_down, 'gripper_close', error_limit=0.01))
    ...
    def target_func_move_front(self, state):
    ...
    def target_func_move_front_down(self, state):
        target_xyz = state['target_object_xyz'].copy()
        target_xyz[-2] += 0.06
        target_xyz[-1] += 0.06
        target = np.hstack(
