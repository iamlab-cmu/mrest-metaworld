...
class SawyerPickAndPlaceMultiTaskGenEnvV23(SawyerXYZEnv):
    ...
    def __init__(self, env_cfg, data_collection=False,):
        ...
        # NOTE: This list should be consistent in how these objects are specified
        # in the XML file.
        self.blocks = SawyerPickAndPlaceMultiTaskGenEnvV23.BLOCKS
        self.objects = SawyerPickAndPlaceMultiTaskGenEnvV23.OBJECTS
    ...
    def get_target_object_collision_shape_for_sampling(self, task, object_names):
        create_centered_polygon_with_halfsize = SawyerPickAndPlaceMultiTaskGenEnvV23.create_centered_polygon_with_halfsize
        objects = []
        obj_th_xy = [0.05, 0.05]
        if task == 'pick':
    ...
    def sample_object_locations(self):
        ...
        for object_name in all_objects:
            ...
            obj_polygon = SawyerPickAndPlaceMultiTaskGenEnvV23.create_centered_polygon_with_halfsize(
    ...
    def reset_model(self):
        ...
        if self.task_info.update_block_colors:
            for block_id in ['A', 'B', 'C', 'D']:
                ...
                block_color = SawyerPickAndPlaceMultiTaskGenEnvV23.BLOCK_COLORS[block_color_name]
                geom_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, f'block{block_id}')
    ...
    def _get_quat_objects(self):
        objects = np.hstack([self.data.xquat[mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, obj_name)]
                             for obj_name in self.get_all_objects()])
        if self.add_buttonpress_obj:
            objects = np.r_[objects, 
                self.data.xquat[mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, 'button')]]
        
        elif self.add_bin_obj:
            objects = np.r_[objects, 
                self.data.xquat[mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, 'bin_goal')]]
