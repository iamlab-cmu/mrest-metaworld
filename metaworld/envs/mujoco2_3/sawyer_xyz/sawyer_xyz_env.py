...
from metaworld.envs.mujoco2_3.mujoco_env import MujocoEnv, _assert_task_is_set
...
class SawyerMocapBase(MujocoEnv, metaclass=abc.ABCMeta):
    ...
    def get_endeff_pos(self):
        bid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, 'robot0_hand')
    ...
    def tcp_center(self):
        ...
        """
        right_finger_pos = self._get_site_pos('robot0_rightEndEffector')
        left_finger_pos = self._get_site_pos('robot0_leftEndEffector')
        tcp_center = (right_finger_pos + left_finger_pos) / 2.0
        return tcp_center

    def get_env_state(self):
        joint_state = np.copy(self.data.qpos)
    ...
    def set_env_state(self, state):
        ...
        self.data.mocap_pos = mocap_pos.copy()
        self.data.mocap_quat = mocap_quat.copy()
...
class SawyerXYZEnv(SawyerMocapBase, metaclass=abc.ABCMeta):
    ...
    def _reset_hand(self, steps=50):
        for _ in range(steps):
            ...
            # self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
            self.data.mocap_pos = self.hand_init_pos.copy()
            self.data.mocap_quat = np.array([1, 0, 1, 0])
            # mujoco.mj_forward(self.model, self.data)
