...
class SawyerPickAndPlaceMultiTaskGenEnvV2(SawyerXYZEnv):
    ...
    def __init__(self, env_cfg, data_collection=False,):
        ...
        # NOTE: This list should be consistent in how these objects are specified
        # in the XML file.
        self.blocks = SawyerPickAndPlaceMultiTaskGenEnvV2.BLOCKS
        self.objects = SawyerPickAndPlaceMultiTaskGenEnvV2.OBJECTS
        ...
        self.object_init_locations = OmegaConf.create({
            'blockA': [0., 0.4, self.block_height],
    ...
    def model_name(self):
        if self.only_use_block_objects:
            if self.add_buttonpress_obj:
                return full_v2_path_for(f'sawyer_xyz/sawyer_pick_place_multitask_onlyblocks_buttonbox.xml')
            elif self.add_bin_obj:
                return full_v2_path_for(f'sawyer_xyz/sawyer_pick_place_multitask_onlyblocks_bin.xml')
            else:
                return full_v2_path_for(f'sawyer_xyz/sawyer_pick_and_place_multitask_gen_onlyblocks.xml')
        else:
            return full_v2_path_for(f'sawyer_xyz/sawyer_pick_and_place_multitask_gen.xml')
    ...
    def get_target_object_collision_shape_for_sampling(self, task, object_names):
        create_centered_polygon_with_halfsize = SawyerPickAndPlaceMultiTaskGenEnvV2.create_centered_polygon_with_halfsize
        objects = []
        obj_th_xy = [0.05, 0.05]
        if task == 'pick':
    ...
    def sample_object_locations(self):
        ...
        for object_name in all_objects:
            ...
            obj_polygon = SawyerPickAndPlaceMultiTaskGenEnvV2.create_centered_polygon_with_halfsize(
    ...
    def reset_model(self):
        ...
        site_id = self.model.site_name2id(place_site_name)
        if 'site' in self.obj_locations_by_name:
            ...
            rgba = np.copy(self.sim.model.site_rgba[site_id])
            rgba[-1] = 0.0
            self.sim.model.site_rgba[site_id] = rgba

        if self.task_info.update_block_colors:
            for block_id in ['A', 'B', 'C', 'D']:
                block_color_name = self.env_cfg[f'block{block_id}_config']['color']
                block_color = SawyerPickAndPlaceMultiTaskGenEnvV2.BLOCK_COLORS[block_color_name]
                geom_id = self.model.geom_name2id(f'block{block_id}')
                self.sim.model.geom_rgba[geom_id] = np.array(block_color)
    ...
    def _set_goal(self):
        ...
        if 'buttonpush' in task_lower:
            ...
            goal = self.get_body_com(self.env_cfg['stack_on_object']).copy()
            goal[0] += 0.08
            ...
            goal = self.sim.model.site_pos[self.model.site_name2id('placeSiteA')]
        elif 'pick' in task_lower:
            goal = self.get_body_com(target_object).copy()
            ...
            pos = self.get_body_com(self.env_cfg['stack_on_object']).copy()
            goal[:2] = pos[:2]
            # goal[-1] = pos[-1] + 0.12
            goal[-1] = pos[-1] + 0.09
        else:
    ...
    def _get_pos_objects(self):
        ...
        objects = np.hstack([self.get_body_com(obj_name)
                             for obj_name in self.get_all_objects()])
        if self.add_buttonpress_obj:
    ...
    def _get_quat_objects(self):
        objects = np.hstack([self.sim.data.get_body_xquat(obj_name)
                             for obj_name in self.get_all_objects()])
        if self.add_buttonpress_obj:
            objects = np.r_[objects, self.sim.data.get_body_xquat('button')]
        
        elif self.add_bin_obj:
            objects = np.r_[objects, self.sim.data.get_body_xquat('bin_goal')]
    ...
    def evaluate_state(self, obs, action):
        ...
        gripper_vel_metric = np.linalg.norm(self.data.get_body_xvelp('hand')) < 0.005
        ...
        if self.data_collection:
            if 'stack' in self.task:
                ...
                gripper_to_goal = np.linalg.norm(obs[:3] - self._target_pos) <= self.error_limit
                success = float(gripper_to_goal and gripper_vel_metric)
    ...
    def compute_reward(self, action, obs):
        ...
        gripper_open_without_block = (obs[3]>0.7)
